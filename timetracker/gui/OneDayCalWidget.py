...
class OneDayCalWidget(QtWidgets.QWidget):
    def __init__(self, parent):
        ...
        calOptions = {}
        calOptions['date'] = datetime.date.today()
        calOptions['calendars']=['primary']
        calOptions['eventBoxColors']=[QColor(0,129,197),QColor(0,129,197)]
        calOptions['showCurrentTime']=True
        ...
        parent.calendar_layout.addWidget(self)
        parent.calendar_groupBox.setLayout(parent.calendar_layout)
    ...
    def calcSpan(self):
        ...
        if self.hasEvents:
            ...
            self.calEnd = datetime.time(hour=18, tzinfo=tz)
            ...
            for cal in self.eventMasterList:
                for event in cal:
                    ...
                    event['relStart'] = (p.parse(event['start'].get(
                        'dateTime')) - self.dtCalStart).total_seconds() / self.calSpan.total_seconds()

        else:

            tz = datetime.datetime.now().tzinfo
            self.calDate = datetime.date.today()
            self.calStart = datetime.time(hour=8, tzinfo=tz)
            self.calEnd = datetime.time(hour=18, tzinfo=tz)
            self.dtCalStart = datetime.datetime.combine(self.calDate, self.calStart)
            self.dtCalEnd = datetime.datetime.combine(self.calDate, self.calEnd)
    ...
    def drawLines(self, qp):
        ...
        if (self.syncedTitleFontSize is None):
            ...
            while (True):
                ...
                r = QtGui.QFontMetrics(f).boundingRect(
                    str(self.calDate.month) + '/' + str(self.calDate.day) + "/" + str(self.calDate.year))
        ...
        qp.drawText((self.width() - self.titleWidth) / 2, self.titleHeight,
                    str(self.calDate.month) + '/' + str(self.calDate.day) + "/" + str(self.calDate.year))
        ...
        for x in range(int(hours + 1)):
            qp.drawLine(0, 25 + self.titleHeight + (self.height() - 50 - self.titleHeight) / hours * x, self.width(),
                        25 + self.titleHeight + (self.height() - 50 - self.titleHeight) / hours * x)
            qp.drawText(5, 40 + self.titleHeight + (self.height() - 50 - self.titleHeight) / hours * x,
                        (self.dtCalStart + datetime.timedelta(hours=x)).strftime('%I%p').lstrip("0").replace(" 0", " "))

        pen = QtGui.QPen(QtCore.Qt.blue, 1, QtCore.Qt.SolidLine)
        qp.setPen(pen)

        if self.showCurrentTime and self.dtCalStart < datetime.datetime.now(
                self.dtCalStart.tzinfo) and self.dtCalEnd > datetime.datetime.now(self.dtCalEnd.tzinfo):
            ...
            qp.drawLine(0, 25 + self.titleHeight + (self.height() - 50 - self.titleHeight) * relHeight, self.width(),
                        25 + self.titleHeight + (self.height() - 50 - self.titleHeight) * relHeight)
    ...
    def drawEvents(self, qp):
        ...
        for counter, cal in enumerate(self.eventMasterList):
            ...
            for event in cal:
                r = QtCore.QRect(60 + counter * (eventWidth + 10),
                                 25 + self.titleHeight + (self.height() - 50 - self.titleHeight) * event['relStart'],
                                 eventWidth, (self.height() - 50 - self.titleHeight) * event['relHeight'])
                qp.drawRect(r)
                qp.drawText(r.left() + 5, r.top() + 2, r.width() - 5, r.height() - 2, QtCore.Qt.TextWordWrap,
                            event['summary'])
    ...
    def setCalStartEnd(self, start, end):
        ...
        for cal in self.eventMasterList:
            for event in cal:
                event['relHeight'] = event['timespan'].total_seconds() / self.calSpan.total_seconds()
                event['relStart'] = (p.parse(
                    event['start'].get('dateTime')) - self.dtCalStart).total_seconds() / self.calSpan.total_seconds()
    ...
    def getEventList(self, calOptions):
        ...
        for cal in calOptions['calendars']:
            ...
            events_result = service.events().list(calendarId='primary', timeMin=morning,
...
if __name__ == '__main__':
    ...
    cal1Options['calendars'] = calList = ['primary','other cal']
    cal1Options['eventBoxColors'] = [QColor(0,129,197),QColor(0,129,197)]
    cal1Options['showCurrentTime'] = True
    cal1 = OneDayCalWidget(cal1Options, Config)
